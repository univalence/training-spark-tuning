...
object exercise_macros {
  ...
  def ignoreExerciseMacro(c: Context)(label: c.Expr[String])(f: c.Tree): c.Expr[Unit] = {
    ...
    c.Expr[Unit](q"""
io.univalence.spark_tuning.internal.exercise_macros.Section(
  path  = $path,
  line  = $line,
  label = $label
).reportOnly($f)
...
""")
  ...
  def commentMacro(c: blackbox.Context)(content: c.Expr[String]): c.Expr[Unit] = {
    ...
    c.Expr[Unit](
      q"""{
  println(
    io.univalence.spark_tuning.internal.exercise_macros.partIndent
    + io.univalence.spark_tuning.internal.exercise_macros.COLOR_GREY
    + "# "
    + $content
  ...
  println(io.univalence.spark_tuning.internal.exercise_macros.partIndent + content)
}"""
    )
  }
